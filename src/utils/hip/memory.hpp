...
namespace util {
    ...
    // -----------------------------------------------------------------------
    // Allocator type selector for main-memory and L3 benchmarks.
    // L1/L2 benchmarks always use hipMalloc and are not affected by this enum.
    // -----------------------------------------------------------------------
    enum class AllocatorType {
        ...
        Malloc             // ::malloc         — CPU heap; accessible from GPU via HMM on MI300A
                           //                    without hipHostRegister. Requires XNACK=1.
    };

    // -----------------------------------------------------------------------
    // allocateMemory<T> — dispatch to the appropriate allocator.
    // -----------------------------------------------------------------------
    template <typename T = uint32_t>
    T* allocateMemory(size_t numElems, AllocatorType type) {
        T* ptr = nullptr;
        ...
        switch (type) {
            ...
            case AllocatorType::HipHostMalloc:
                ...
                // Fix the AMD allocation to coarse-grained coherence so that
                // HIP_HOST_COHERENT cannot silently change benchmark semantics.
                util::hipCheck(hipHostMalloc(&ptr, bytes, hipHostMallocNonCoherent));
#else
                // hipHostMallocNonCoherent is an AMD extension. Preserve the portable
                // default for non-AMD builds.
                util::hipCheck(hipHostMalloc(&ptr, bytes, hipHostMallocDefault));
#endif
                break;

            case AllocatorType::Malloc:
                // Accessible from GPU via HMM on MI300A without hipHostRegister. Requires XNACK=1.
    ...
    // -----------------------------------------------------------------------
    // freeMemory — dispatch matching free for each allocator.
    // -----------------------------------------------------------------------
